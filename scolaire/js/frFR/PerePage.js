...
function postPereScolaire($formulaireValeurs, success, error) {
	var vals = {};
	if(success == null) {
	...
	var valeurPereId = $formulaireValeurs.find('.valeurPereId').val();
	if(valeurPereId != null && valeurPereId !== '')
		vals['pereId'] = valeurPereId;
...
function patchPereScolaire($formulaireFiltres, $formulaireValeurs, success, error) {
	...
	var removePereId = $formulaireFiltres.find('.removePereId').val() === 'true';
	var setPereId = removePereId ? null : $formulaireValeurs.find('.setPereId').val();
	if(removePereId || setPereId != null && setPereId !== '')
		vals['setPereId'] = setPereId;
	var addPereId = $formulaireValeurs.find('.addPereId').val();
	if(addPereId != null && addPereId !== '')
		vals['addPereId'] = addPereId;
	var removePereId = $formulaireValeurs.find('.removePereId').val();
	if(removePereId != null && removePereId !== '')
		vals['removePereId'] = removePereId;
...
function patchPereScolaireFiltres($formulaireFiltres) {
	...
	var filtrePereId = $formulaireFiltres.find('.valeurPereId').val();
	if(filtrePereId != null && filtrePereId !== '')
		filtres.push({ name: 'fq', value: 'pereId:' + filtrePereId });
...
function deletePereScolaire(pk) {
	$.ajax({
		url: '/api/pere/' + id
		, dataType: 'json'
		, type: 'DELETE'
...
function recherchePereScolaire($formulaireFiltres, success, error) {
...
function suggerePereScolaireInscriptionCles($formulaireFiltres, $list) {
	success = function( data, textStatus, jQxhr ) {
		$list.empty();
		$.each(data['list'], function(i, o) {
			...
			$a.append($i);
			$a.append($span);
			var pk = parseInt($('#PereScolaireForm :input[name="pk"]').val());
			var val = o['pereCles'];
			var checked = Array.isArray(val) ? val.includes(pk) : val == pk;
			var $input = $('<input>');
			$input.attr('id', 'GET_inscriptionCles_' + pk + '_pereCles_' + o['pk']);
			$input.attr('class', 'w3-check ');
			$input.attr('onchange', "var $input = $('#GET_inscriptionCles_" + pk + "_pereCles_" + o['pk'] + "'); patchPereScolaireVals([{ name: 'fq', value: 'pk:" + pk + "' }], { [($input.prop('checked') ? 'add' : 'remove') + 'InscriptionCles']: \"" + o['pk'] + "\" }, function() { patchInscriptionScolaireVals([{ name: 'fq', value: 'pk:" + o['pk'] + "' }], {}, function() { ajouterLueur($input); }, function() { ajouterErreur($input); } ); } ); ");
