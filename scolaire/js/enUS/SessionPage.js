...
async function putcopySchoolSession($formValues, pk, success, error) {
	...
	var valueSeasonKeyClear = $formValues.find('input.seasonKey_clear:checked').val();
	if(valueSeasonKeyClear != null && valueSeasonKeyClear)
		vals['seasonKey'] = null;
	else if(valueSeasonKeyClear != null && valueSeasonKeyClear)
		vals['seasonKey'] = valueSeasonKey;
...
async function patchSchoolSession($formFilters, $formValues, pk, success, error) {
	...
	var setMoment = setSessionStartDate ? moment(setSessionStartDate, 'MM/DD/YYYY') : null; 
	var addMoment = addSessionStartDate ? moment(addSessionStartDate, 'MM/DD/YYYY') : null; 
	if(setMoment) { 
		var s = setMoment.format('YYYY-MM-DD'); 
		setSessionStartDate = s;
	} 
	if(addMoment) { 
		var s = addMoment.format('YYYY-MM-DD'); 
		addSessionStartDate = s;
	} 
	...
	var setMoment = setSessionEndDate ? moment(setSessionEndDate, 'MM/DD/YYYY') : null; 
	var addMoment = addSessionEndDate ? moment(addSessionEndDate, 'MM/DD/YYYY') : null; 
	if(setMoment) { 
		var s = setMoment.format('YYYY-MM-DD'); 
		setSessionEndDate = s;
	} 
