...
function postSchoolChild($formValues) {
	var vals = {};
	...
	var valueChildId = $formValues.find('.valueChildId').val();
	if(valueChildId != null && valueChildId !== '')
		vals['childId'] = valueChildId;
...
function patchSchoolChild($formFilters, $formValues, success, error) {
	...
	var removeChildId = $formFilters.find('.removeChildId').val() === 'true';
	var setChildId = removeChildId ? null : $formValues.find('.setChildId').val();
	if(removeChildId || setChildId != null && setChildId !== '')
		vals['setChildId'] = setChildId;
	var addChildId = $formValues.find('.addChildId').val();
	if(addChildId != null && addChildId !== '')
		vals['addChildId'] = addChildId;
	var removeChildId = $formValues.find('.removeChildId').val();
	if(removeChildId != null && removeChildId !== '')
		vals['removeChildId'] = removeChildId;
...
function patchSchoolChildFilters($formFilters) {
	...
	var filterChildId = $formFilters.find('.valueChildId').val();
	if(filterChildId != null && filterChildId !== '')
		filters.push({ name: 'fq', value: 'childId:' + filterChildId });
...
function searchSchoolChild($formFilters, success, error) {
...
function suggestSchoolChildEnrollmentKeys($formFilters, $list) {
	success = function( data, textStatus, jQxhr ) {
		...
		$.each(data['list'], function(i, o) {
			...
			$input.attr('onchange', "var $input = $('#GET_enrollmentKeys_" + pk + "_childKey_" + o['pk'] + "'); patchSchoolChildVals([{ name: 'fq', value: 'pk:" + pk + "' }], { [($input.prop('checked') ? 'add' : 'remove') + 'EnrollmentKeys']: \"" + o['pk'] + "\" }, function() { patchSchoolEnrollmentVals([{ name: 'fq', value: 'pk:" + o['pk'] + "' }], {}, function() { addGlow($input); }, function() { addError($input); } ); } ); ");
