...
function postSchoolMom($formValues, success, error) {
	var vals = {};
	if(success == null) {
	...
	var valueMomId = $formValues.find('.valueMomId').val();
	if(valueMomId != null && valueMomId !== '')
		vals['momId'] = valueMomId;
...
function patchSchoolMom($formFilters, $formValues, success, error) {
	...
	var removeMomId = $formFilters.find('.removeMomId').val() === 'true';
	var setMomId = removeMomId ? null : $formValues.find('.setMomId').val();
	if(removeMomId || setMomId != null && setMomId !== '')
		vals['setMomId'] = setMomId;
	var addMomId = $formValues.find('.addMomId').val();
	if(addMomId != null && addMomId !== '')
		vals['addMomId'] = addMomId;
	var removeMomId = $formValues.find('.removeMomId').val();
	if(removeMomId != null && removeMomId !== '')
		vals['removeMomId'] = removeMomId;
...
function patchSchoolMomFilters($formFilters) {
	...
	var filterMomId = $formFilters.find('.valueMomId').val();
	if(filterMomId != null && filterMomId !== '')
		filters.push({ name: 'fq', value: 'momId:' + filterMomId });
...
function searchSchoolMom($formFilters, success, error) {
...
function suggestSchoolMomEnrollmentKeys($formFilters, $list) {
	success = function( data, textStatus, jQxhr ) {
		$list.empty();
		$.each(data['list'], function(i, o) {
			...
			$a.append($i);
			$a.append($span);
			var pk = parseInt($('#SchoolMomForm :input[name="pk"]').val());
			var val = o['momKeys'];
			var checked = Array.isArray(val) ? val.includes(pk) : val == pk;
			var $input = $('<input>');
			$input.attr('id', 'GET_enrollmentKeys_' + pk + '_momKeys_' + o['pk']);
			$input.attr('class', 'w3-check ');
			$input.attr('onchange', "var $input = $('#GET_enrollmentKeys_" + pk + "_momKeys_" + o['pk'] + "'); patchSchoolMomVals([{ name: 'fq', value: 'pk:" + pk + "' }], { [($input.prop('checked') ? 'add' : 'remove') + 'EnrollmentKeys']: \"" + o['pk'] + "\" }, function() { patchSchoolEnrollmentVals([{ name: 'fq', value: 'pk:" + o['pk'] + "' }], {}, function() { addGlow($input); }, function() { addError($input); } ); } ); ");
